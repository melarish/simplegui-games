# run at http://www.codeskulptor.org/#user39_hzGc079Bl4_3.py
# Asteroids game
import simplegui
import math
import random
...
MAX_LIVES = 3                            
rock_max_speed = 1
alien_missile_cooldown = 40
alien_spawn_interval = 200
time = 0
score = 0
lives = MAX_LIVES
explosion_group = set()
...
def init(): # start/restart
    global started, rock_group, missile_group, alien_missile_group
    started = False
    alien_ship.active = False
    rock_group = set()
    missile_group = set()
...
# alien ship image
alien_ship_info = ImageInfo([63, 75], [125, 150], 63)
alien_ship_image = simplegui.load_image("http://news.rice.edu/wp-content/uploads/2012/12/1214_COURSERA-mug.jpg")
...
# alien missile image
alien_missile_info = ImageInfo([14,25], [27,49], 14, 50)
alien_missile_image = simplegui.load_image("http://i.imgur.com/ekGCaHp.jpg")
...
class Alienship(Ship):
    def update(self):
        self.alien_spawner()
        self.pos[0] = (self.pos[0] + 5) % WIDTH
        if self.active:
            self.shoot()
        
    def shoot(self):
        if time % alien_missile_cooldown == 0:
            a_missile = Sprite([self.pos[0],self.pos[1] * 2],
                           [0,5], 0, 0, alien_missile_image, alien_missile_info, missile_sound)
            alien_missile_group.add(a_missile)
    ...
    def alien_spawner(self):
        if started and not self.active:
            if time % alien_spawn_interval == 0:
                self.active = True
...
def draw(canvas):
    global time, started, lives, score, rock_max_speed, alien_ship
    ...
    my_ship.draw(canvas)
    my_ship.update()
    alien_ship.draw(canvas)
    alien_ship.update()
    process_sprite_group(rock_group, canvas)
    process_sprite_group(missile_group, canvas)
    process_sprite_group(alien_missile_group, canvas)
    process_sprite_group(explosion_group, canvas)
    # if rock or alien missile hits ship
    if group_collide(rock_group,my_ship) or group_collide(alien_missile_group,my_ship): 
        lives -= 1
        if lives == 0: # game over
            timer.stop()
            init()
    if group_collide(missile_group,alien_ship): 
        score += 10
        alien_ship.active = False
...
alien_ship = Alienship([WIDTH / 8, HEIGHT / 8], [0, 0], 0, alien_ship_image, alien_ship_info)
